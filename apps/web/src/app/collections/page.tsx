...
export default async function CollectionsIndex({
  ...
  const { page: initialListing, facets: baseFacets } = await fetchListingWithFallback({
    mode: 'all',
    first: pageSize,
  ...
  const initialFacets = deriveCollectionColors(baseFacets, []);
  const totalCount =
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-sm text-muted-foreground">
                Browse the full Shopify catalog with filters and sorting.
              </p>
            </div>
