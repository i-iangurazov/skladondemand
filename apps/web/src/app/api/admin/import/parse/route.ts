...
import type { CsvMapping, ImportParseResult, PriceMode } from '@/lib/importer/types';
...
export async function POST(request: Request) {
  ...
  const priceModeRaw = formData.get('priceMode');
  const priceMode: PriceMode = priceModeRaw === 'wholesale' ? 'wholesale' : 'retail';
  ...
  if (isCsvFile(file)) {
    ...
    if (mappingRaw) {
      ...
      mapping = suggestCsvMapping(parsed.headers, priceMode);
    }
    ...
    await logAdminAction({
      ...
      importJobId: job.id,
      metadata: { fileName: file.name, fileSize: file.size, rows: normalized.rows.length },
    });
    ...
    result = {
      ...
      mapping,
      needsReviewCount: normalized.rows.filter((row) => row.source?.needsReview).length,
    };
  }
